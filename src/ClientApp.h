...
public:
  ...
  static std::string encode(std::string_view message, OpCode opCode = TEXT,
                            bool fin = true) {
    std::string frame;
    char header[14];
    size_t headerSize = 2;
    ...
    header[0] = static_cast<char>((fin ? 0x80 : 0x00) |
  ...
  static size_t decode(const char *data, size_t size, std::string &outMessage,
                       OpCode &outOpCode, bool &outFin) {
    if (size < 2)
      return 0;
    ...
    if (length == 126) {
      if (size < 4)
        return 0;
      length = (static_cast<unsigned char>(data[2]) << 8) |
    ...
    if (masked) {
      if (size < headerSize + 4 + length)
        return 0;
      const unsigned char *mask =
          reinterpret_cast<const unsigned char *>(data + headerSize);
      headerSize += 4;
...
private:
  static void maskData(char *data, size_t length, const unsigned char mask[4]) {
...
#ifdef __AVX2__
    __m256i avxMaskVec =
        _mm256_set_epi8(static_cast<char>(mask[3]), static_cast<char>(mask[2]),
                        static_cast<char>(mask[1]), static_cast<char>(mask[0]),
                        static_cast<char>(mask[3]), static_cast<char>(mask[2]),
                        static_cast<char>(mask[1]), static_cast<char>(mask[0]),
                        static_cast<char>(mask[3]), static_cast<char>(mask[2]),
                        static_cast<char>(mask[1]), static_cast<char>(mask[0]),
                        static_cast<char>(mask[3]), static_cast<char>(mask[2]),
    ...
    for (; i + 31 < length; i += 32) {
      __m256i dataVec =
          _mm256_loadu_si256(reinterpret_cast<__m256i *>(data + i));
...
#ifdef __SSE2__
    __m128i sseMaskVec =
        _mm_set_epi8(static_cast<char>(mask[3]), static_cast<char>(mask[2]),
                     static_cast<char>(mask[1]), static_cast<char>(mask[0]),
                     static_cast<char>(mask[3]), static_cast<char>(mask[2]),
                     static_cast<char>(mask[1]), static_cast<char>(mask[0]),
                     static_cast<char>(mask[3]), static_cast<char>(mask[2]),
...
public:
  ClientWebSocket(WebSocketClientBehavior *b, int fd, void * /*s*/)
      : behavior(b), socket_fd(fd)
#ifdef LIBUS_USE_OPENSSL
        ,
  ...
  void send(std::string_view message,
            WebSocketFrame::OpCode opCode = WebSocketFrame::TEXT) {
...
private:
...
#ifdef LIBUS_USE_OPENSSL
      if (ssl) {
        ssize_t written = SSL_write(static_cast<SSL *>(ssl), sendBuffer.data(),
                                    static_cast<int>(sendBuffer.size()));
        if (written > 0) {
          sendBuffer.erase(sendBuffer.begin(), sendBuffer.begin() + written);
        } else if (written < 0) {
          int ssl_err =
              SSL_get_error(static_cast<SSL *>(ssl), static_cast<int>(written));
...
#endif
        ssize_t written =
            write(socket_fd, sendBuffer.data(), sendBuffer.size());
        if (written > 0) {
          sendBuffer.erase(sendBuffer.begin(), sendBuffer.begin() + written);
  ...
  void processReceiveBuffer() {
    ...
    while (offset < receiveBuffer.size()) {
      ...
      bool fin;
      size_t consumed = WebSocketFrame::decode(receiveBuffer.data() + offset,
    ...
    if (readOffset > receiveBuffer.size() / 2) {
      receiveBuffer.erase(receiveBuffer.begin(),
                          receiveBuffer.begin() +
                              static_cast<ptrdiff_t>(readOffset));
      readOffset = 0;
    }
  }
};
...
public:
  ...
  bool connect(std::string url) {
...
#ifdef LIBUS_USE_OPENSSL
    if (use_ssl) {
      sent = SSL_write(static_cast<SSL *>(ssl), handshake.data(),
                       handshake.size());
    } else
#endif
    {
    ...
    while (response_size < sizeof(response_buffer)) {
#ifdef LIBUS_USE_OPENSSL
      if (use_ssl) {
        n = SSL_read(static_cast<SSL *>(ssl), response_buffer + response_size,
                     sizeof(response_buffer) - response_size);
        if (n < 0) {
          int ssl_err =
              SSL_get_error(static_cast<SSL *>(ssl), static_cast<int>(n));
...
#endif
      {
        n = read(socket_fd, response_buffer + response_size,
                 sizeof(response_buffer) - response_size);
        if (n < 0) {
      ...
      if (response_size >= 4 &&
          memcmp(response_buffer + response_size - 4, "\r\n\r\n", 4) == 0)
        break;
    }
